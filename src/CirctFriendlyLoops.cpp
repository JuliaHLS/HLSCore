...
template <typename LoopType>
static void lowerLinalgToLoopsImplCirctFriendly(Operation *enclosingOp) {
  MLIRContext *context = enclosingOp->getContext();
  RewritePatternSet patterns(context);
  patterns.add<LinalgRewritePattern<LoopType>>(context);
  memref::DimOp::getCanonicalizationPatterns(patterns, context);
  ...
  // Just apply the patterns greedily.
  (void)applyPatternsGreedily(enclosingOp, std::move(patterns), config);
}
...
linalgOpToAffineLoops(RewriterBase &rewriter, LinalgOp linalgOp) {
...
FailureOr<LinalgLoops>
linalgOpToParallelLoops(RewriterBase &rewriter,
                                      LinalgOp linalgOp) {
...
struct LowerToAffineLoopsCirctFriendly
    : public impl::ConvertLinalgToAffineLoopsPassBase<LowerToAffineLoopsCirctFriendly> {
  using impl::ConvertLinalgToAffineLoopsPassBase<
      LowerToAffineLoopsCirctFriendly>::ConvertLinalgToAffineLoopsPassBase;
  void getDependentDialects(DialectRegistry &registry) const override {
    registry.insert<memref::MemRefDialect>();
  }
  void runOnOperation() override {
    lowerLinalgToLoopsImplCirctFriendly<affine::AffineForOp>(getOperation());
  }
};

}

std::unique_ptr<mlir::Pass> HLSCore::passes::createLowerLinalgToAffineCirctFriendly() {
    return std::make_unique<HLSCore::passes::LowerToAffineLoopsCirctFriendly>();
}
